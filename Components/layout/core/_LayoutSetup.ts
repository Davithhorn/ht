...
class LayoutSetup {
    ...
    private static initLayoutSettings(config: ILayout): ILayout {
        ...
        if (isBrowser) {
            // clear body classes
            const bodyClasses = document.body.classList.value.split(' ').filter(cssClass => cssClass)
            ...
            document.body.setAttribute('id', 'main_app_body')
            document.body.setAttribute('data-kt-app-layout', updatedConfig.layoutType)
            document.body.setAttribute('data-kt-name', 'next')
