...
class ThemeMode {
  ...
  public setMode = (mode: Mode, menuMode: Mode | ''): void => {
    ...
    if (activeMenuItem && typeof localStorage !== 'undefined') {
  ...
  public getMenuMode = (): Mode | '' => {
    ...
    const ls = typeof localStorage !== 'undefined' ? localStorage.getItem(themeMenuModeLSKey) : null
    return (ls as Mode) || ''
  }
  ...
  private setActiveMenuItem = (item: HTMLElement): void => {
    ...
    if (typeof localStorage !== 'undefined' && menuMode) {
      localStorage.setItem(themeMenuModeLSKey, menuMode)
    }
  }
